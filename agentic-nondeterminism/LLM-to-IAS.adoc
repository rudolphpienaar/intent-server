...
.Generic neural network diagram. For the present argument, the essential feature is that the forward pass is explicit and deterministic once the parameters are fixed.
image::assets/images/neural-network.svg[Neural network block diagram,align=center,width=85%]

This deterministic and often effectively compressive forward map naturally raises the inverse question: can one move back from the compressed representation to an original input consistent with it? Generative modeling is one answer to that question. But because the forward map is generally many-to-one on the data distribution, generation does not invert the network uniquely; it parameterizes a conditional distribution over admissible outputs and samples from that distribution.

=== The Probabilistic Inverse

Let stem:[g : \mathbb{R}^d \to \mathbb{R}^k] denote the deterministic forward map of a network, so that
...
This naturally suggests the inverse question of whether one can move back from stem:[z] to stem:[x] by an inverse map stem:[g^{-1}], i.e.
...
In general, however, such an inverse is not available. The forward map is many-to-one on the data distribution, so distinct inputs can map to the same or nearby representations. For a given representation stem:[z], the preimage

[stem]
++++
\mathcal{P}(z) = \{ x \in \mathbb{R}^d \mid g(x) = z \}
++++

is large for a typical stem:[z]. There is therefore no unique antecedent to recover. The only meaningful reverse object is not a function stem:[g^{-1}(z)] but a conditional distribution over the compatible preimage set. Generative modeling is the mechanical process of parameterizing such a conditional distribution, written here as stem:[p_{\theta}(x \mid z)] pass:[\cite{brown2020,vaswani2017,kingma2013vae}]. Sampling then draws one member of the class:
...
Any decoding rule, whether stochastic (temperature/top-k/top-p) or even greedy argmax, chooses an element from the compatible set rather than inverting stem:[g] to a single input. Determinism would require collapsing stem:[p_{\theta}(x \mid z)] to a single point, which sacrifices coverage and fidelity. Because the mapping stem:[x \mapsto z] is many-to-one, the reverse direction is inherently probabilistic.

== Why Exact Inversion Does Not Rescue Correctness

The argument above establishes why the generative step is probabilistic. But even if one sets that point aside and asks whether exact inversion would solve the correctness problem, the answer is still no. The issue is not merely that a representation stem:[z] admits many compatible antecedents. It is also that only a small subset of those antecedents are semantically, clinically, or procedurally acceptable.

For a given representation stem:[z], define the compatible preimage set

[stem]
++++
\mathcal{S}(z) = \{\, x \mid g(x) \approx z \,\} .
++++

This is the approximate counterpart of stem:[\mathcal{P}(z)], relaxing exact equality to neighborhood-level compatibility in representation space.

A parameterized model defines a conditional distribution stem:[p_{\theta}(x \mid z)] supported on stem:[\mathcal{S}(z)]. Within stem:[\mathcal{S}(z)], only a small observer-validated subset stem:[\mathcal{G}(z) \subset \mathcal{S}(z)] corresponds to clinically acceptable outputs.

The probability mass assigned to clinically acceptable outputs for stem:[z] is

[stem]
++++
P_{\text{correct}}(z) = \int_{x \in \mathcal{G}(z)} p_{\theta}(x \mid z)\, dx ,
++++

and stem:[1 - P_{\text{correct}}(z)] is the mass on antecedents that are mathematically admissible or probabilistically plausible but clinically wrong. This point is logically independent of the inverse problem itself: even a deterministic selection rule over stem:[\mathcal{S}(z)] would not guarantee that the selected element lies in stem:[\mathcal{G}(z)].
...
By contrast, practical architectures are deliberately non-square at key layers: encoders reduce stem:[n \to k] with stem:[k \ll n], and even decoder blocks introduce rank-deficient projections (e.g., attention projections of size stem:[d_{\text{model}} \times d_k], embedding tables with fewer rows than token combinations). These maps are not invertible by construction, and any inverse-like operation must therefore operate over a preimage set rather than a single exact antecedent.
