...
This paper argues that this view is fundamentally mistaken. Hallucination is not a superficial defect of generative models that can be removed through better prompting or insistence on explicit steps. It is a structural property of probabilistic systems operating over a very large space of possible action sequences pass:[\cite{vaswani2017,ji2023survey,maynez2020faithfulness}]. When the number of exposed tools increases and the number of generated reasoning steps grows, hallucination risk is not mitigated; it is amplified.
...
This expression is nonlinear, but it is entirely explicit and mechanical once the weights are fixed. That is the key point: the forward pass is not mysterious, not interpretive, and not stochastic. It is simply a deterministic composition of weighted sums and pointwise nonlinearities. The crucial structural feature, however, is not nonlinearity by itself but compression: the network maps four input degrees of freedom to two output coordinates through successive non-square layers. For that reason the map is not injective in general, and a unique inverse is therefore not available in general. This deterministic and compressive forward map naturally raises the inverse question: can one move back from the representation to an original input consistent with it? Generative modeling is one answer to that question. But because the forward map is generally many-to-one on the data distribution, generation does not invert the network uniquely; it parameterizes a conditional distribution over admissible outputs and samples from that distribution.

=== The Probabilistic Inverse

Abstracting now from this concrete stem:[4 \to 3 \to 2] example, let the deterministic forward map be written in general form as follows.

Let stem:[\mathcal{X} \subseteq \mathbb{R}^d] denote the input space and stem:[\mathcal{Z} \subseteq \mathbb{R}^k] the output, or representation, space of the forward map. Let stem:[g_{\theta} : \mathcal{X} \to \mathcal{Z}] denote the deterministic forward map implemented by a network with fixed parameters stem:[\theta], so that
...
This naturally suggests the inverse question of whether one can move back from stem:[\mathbf{z}] to stem:[\mathbf{x}] by an inverse map stem:[g_{\theta}^{-1}], i.e.
...
In general, however, such an inverse is not available. The forward map is many-to-one on the data distribution, so distinct inputs can map to the same or nearby representations. For a given representation stem:[\mathbf{z}], the preimage
...
is large for a typical stem:[\mathbf{z}]. In the concrete stem:[4 \to 3 \to 2] example above, this is the familiar fact that two output coordinates cannot, in general, uniquely recover four input coordinates: the forward map is compressive and therefore non-injective. There is therefore no unique antecedent to recover. The only meaningful reverse object is not a function stem:[g_{\theta}^{-1}(\mathbf{z})] but a conditional distribution over the compatible preimage set. Generative modeling is the mechanical process of parameterizing such a conditional distribution, written here as stem:[p_{\theta}(\mathbf{x} \mid \mathbf{z})] pass:[\cite{brown2020,vaswani2017,kingma2013vae}]. Sampling then draws one member of the class:
...
Even when the emitted code is syntactically valid and apparently deterministic, its semantics may be incorrect. It may invoke the wrong tools, apply the wrong thresholds, or depend on incorrectly retrieved or incorrectly parsed evidence. None of this is prevented by transparency.
